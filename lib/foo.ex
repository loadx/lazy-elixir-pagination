...
#   def get_all_pages(per_page) when per_page > 1 do
#     start_fn = fn -> get_next_pages(all_pages(), 1, [], per_page - 1) end

#     next_fn = fn
#       {:more, acc} ->
#         # the most contrived key ever
#         key_num = Enum.at(acc, 0) |> Map.get(:row)
#         {[Enum.reverse(acc)], get_next_pages(all_pages(), key_num+1, acc, per_page - 1)}
